...
Query.prototype = new Filterable();

Query.prototype.exec = async function (next) {
  debug('exec query for ', this.query);
  const that = this;
  this.Model._emit('model:query', 'exec:start', {
    'event': {'query': this, 'callback': next},
    'actions': {
      updateCallback (fn) {
        next = fn;
      }
  ...
  if (this.operators) {
    this.filterExpression = await this._evaluate(this.operators);
    queryReq.FilterExpression = this.filterExpression;
    queryReq.ExpressionAttributeNames = this.expressionAttributeNames;
    if (Object.keys(this.expressionAttributeValues).length) {
      queryReq.ExpressionAttributeValues = this.expressionAttributeValues;
    }
  }
  ...
  function query () {
    ...
    async function queryOne () {
      ...
      const shouldContinueRequestPre = await that.Model._emit(
        'model:query',
        'request:pre',
        {
          'event': {'query': that, 'callback': next, queryReq},
          'actions': {
            updateQueryReq (req) {
              queryReq = req;
            }
      ...
      Model$.base.ddb().query(queryReq, async (err, data) => {
        const shouldContinueRequestPost = await that.Model._emit(
          'model:query',
          'request:post',
          {
            'event': {'query': that, 'callback': next, data, 'error': err},
            'actions': {
              updateError (error) {
                err = error;
              },
