...
Scan.prototype.exec = async function (next) {
  ...
  if (this.operators) {
    ...
    scanReq.FilterExpression = this.filterExpression;
    scanReq.ExpressionAttributeNames = this.expressionAttributeNames;
    if (Object.keys(this.expressionAttributeValues).length) {
      scanReq.ExpressionAttributeValues = this.expressionAttributeValues;
    }
  }
